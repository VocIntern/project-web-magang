                ...
                <div class="card mb-4">
                        ...
                        <div class="d-flex align-items-center mb-4">
                            <div class="company-logo me-3 d-flex align-items-center justify-content-center text-success">
                                @if ($magang->perusahaan->logo)
                                    <img src="{{ asset('storage/' . $magang->perusahaan->logo) }}"
                                        alt="{{ $magang->perusahaan->nama_perusahaan }}" class="img-fluid rounded">
                                @else
                                    {{ substr($magang->perusahaan->nama_perusahaan, 0, 3) }}
                                @endif
                            </div>
                            <div>
                                ...
                                <div>
                                    <span class="badge bg-light text-dark me-2 mb-1"><i class="bi bi-geo-alt"></i>
                                        {{ $magang->lokasi }}</span>
                                    <span class="badge bg-light text-dark me-2 mb-1"><i class="bi bi-briefcase"></i>
                                        {{ $magang->bidang }}</span>
                                    <span class="badge bg-light text-dark mb-1"><i class="bi bi-people"></i> Kuota:
                                        {{ $magang->kuota }}</span>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <p class="mb-0">
                                                ...
                                                {{ \Carbon\Carbon::parse($magang->tanggal_selesai)->format('d M Y') }}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <h6 class="mb-1"><i class="bi bi-clock-history"></i> Durasi</h6>
                                            <p class="mb-0">
                                                {{ round(\Carbon\Carbon::parse($magang->tanggal_mulai)->diffInMonths($magang->tanggal_selesai)) }}
                                                bulan</p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="card-footer bg-white">
                        <div class="d-grid gap-2">
                            ...
                            @else
                                <a href="{{ route('mahasiswa.magang.apply.form', $magang->id) }}"
                                    class="btn btn-success">Lamar Sekarang</a>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card-body">
                            @foreach ($similarMagang as $similar)
                                <div class="mb-3 pb-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                                    <h6 class="mb-1">
                                        <a href="{{ route('mahasiswa.magang.show', $similar->id) }}"
                                            class="text-decoration-none auth-link">{{ $similar->judul }}</a>
                                    </h6>
                                    <p class="mb-1 text-muted small">{{ $similar->perusahaan->nama_perusahaan }}</p>
                                    <div class="d-flex align-items-center">
                                        <span class="badge bg-light text-dark me-2"><i class="bi bi-geo-alt"></i>
                                            {{ $similar->lokasi }}</span>
                                        <span class="badge bg-light text-dark"><i class="bi bi-calendar"></i>
                                            {{ \Carbon\Carbon::parse($similar->tanggal_mulai)->format('d M Y') }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
